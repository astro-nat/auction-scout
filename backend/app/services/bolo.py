...
_CACHE_DIR = Path(os.environ.get("APP_CACHE_DIR", "/tmp/auction-scout-cache"))
...
DEFAULT_BOLO_PATHS: List[Path] = [
    ...
    _DATA_DIR / "fishing_reels_bolo.json",
    _DATA_DIR / "printer_ink_bolo.json",
    _DATA_DIR / "computer_parts_bolo.json",
    ...
    _DATA_DIR / "precious_metals_bolo.json",
]
...
_BOLO_MATCH_CACHE_PATH = _CACHE_DIR / "bolo_match_cache.json"
# Matcher-logic version — folded into the cache fingerprint so a change
# to the MATCHING CODE (not just the JSON brand files) also invalidates
# the on-disk match cache. BUMP THIS whenever match semantics change,
# else previously-cached lots keep their stale verdicts.
#   v2 (2026-08-04): tightened model-phrase proximity to gap-1 and made
#      all-digit tokens reject a leading hyphen (killed the Cartier
#      "Tank box", Zenith "Defy box", and Sterling ".925"/"C-925" false
#      positives).
...
def _whole_word_pattern(literal: str, max_gap_words: int = 6) -> re.Pattern:
    ...
    """
    # Decimal metal hallmark (".925", ".999", ".585", ".750") — require
    # the LITERAL leading period so a bare "925" inside a price
    # ("$925.00"), a hyphenated code ("C-925"), or an address can't
    # false-match. Real hallmarks carry the period; the period-less
    # spellings are covered by their own aliases ("925 silver",
    # "925 sterling", "s925"). Without this, ".925" tokenized down to a
    # bare "925" and matched every "Retail Value $925.00" line — a
    # vacation-package lot got tagged Sterling silver (BOLO scan 8/4).
    _hallmark = re.fullmatch(r"\.\d{3,4}", literal.strip())
    if _hallmark:
        return re.compile(
            rf"(?<![A-Za-z0-9.]){re.escape(literal.strip())}(?![A-Za-z0-9])",
            re.IGNORECASE,
        )
    ...
    if len(tokens) == 1:
        ...
        # All-digit tokens (silver hallmarks like "925", "999", karat
        # codes) additionally reject a LEADING HYPHEN so an alphanumeric
        # code such as a comic cover's "C-925" mark can't satisfy the
        # token. The plain boundary treats "-" as a word break and would
        # match — but "C-925" is a code, not a standalone hallmark. A
        # real ".925" (leading period) or " 925 " still matches.
        left = (
            r"(?<![A-Za-z0-9-])" if tokens[0].isdigit()
            else r"(?<![A-Za-z0-9])"
        )
        return re.compile(
            rf"{left}{token_re}(?![A-Za-z0-9])",
            re.IGNORECASE,
        )
...
class BoloMatcher:
        ...
        for entry in data.get("brands", []) or []:
            ...
            for m in models:
                if not isinstance(m, str) or len(m) < 4:
                    continue
                # Models are contiguous catalog phrases ("El Primero box",
                # "Cartier red box", "IWC Portuguese box"). Match them
                # NEAR-ADJACENT (gap 1) instead of the default 6-word
                # proximity: a generic tail like "box"/"papers" must not
                # bridge to an unrelated first word 5 words away. Without
                # this, the Cartier model "Tank box" matched "…Submersible
                # Tank De-Icer … inside original box" and the Zenith model
                # "Defy box" matched a TYCO set's "…DEFY GRAVITY … the box".
                # Genuine lots that name the brand still match via the
                # brand alias (the wide-gap path), so recall is preserved.
                model_pats.append((m, _whole_word_pattern(m, max_gap_words=1)))
    ...
    def _fingerprint(self) -> str:
        ...
        # loaded) also invalidates the cache. Prefix the matcher-logic
        # version so a change to the matching CODE busts the cache too —
        # mtimes alone only track the JSON brand files, not this module.
        material = (
            f"v{_MATCHER_LOGIC_VERSION}|"
